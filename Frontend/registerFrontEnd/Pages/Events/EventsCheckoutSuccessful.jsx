...
const EMPTY_REMINDER_FREQUENCIES =
...
/*
 * Returns the current calendar date in the same timezone
 * used by the event reminder cron. This prevents a browser
 * in another timezone from exposing an already-expired
 * reminder option.
 */
...
/*
 * Matches the backend cron's calendar-month behavior.
 * For example, March 31 minus one month becomes
 * February 28 or 29.
 */
...
const EventCheckoutSuccess = () => {
  ...
  /*
   * A timing is available only when its due date is today
   * or later for every occurrence in this purchase.
   *
   * The backend stores one set of timing flags across all
   * submitted occurrence IDs. Requiring every occurrence to
   * qualify prevents an overdue reminder from being sent
   * immediately for an earlier date.
   */
  const availableReminderFrequencies =
    useMemo(() => {
      const todayDateOnly =
        getTodayDateOnly(
          DEFAULT_EVENT_TIMEZONE
      ...
      return REMINDER_FREQUENCY_OPTIONS
        .reduce(
          (
            availability,
            option
  ...
  /*
   * Checkout details arrive asynchronously. Once they do,
   * initially select only reminder timings that have not
   * passed. Expired timings remain false.
   */
  useEffect(() => {
    setReminderFrequencies(
      (previous) => {
  ...
  const submitReminderSubscription =
    async (submitEvent) => {
      ...
      const selectedFrequencyOptions =
        availableFrequencyOptions.filter(
          (option) =>
            reminderFrequencies[
              option.key
            ]
    ...
    <main className="event-success-page">
      <section className="event-success-shell">
        ...
        <section className="event-success-updates">
          <div>
            ...
            <p>
              ...
              1
                ...
                : "the event"}
              . Reminder timings that
              have already passed are
          ...
          {reminderEligibleDates.length ===
            0 ||
          availableFrequencyOptions.length ===
            0 ? (
